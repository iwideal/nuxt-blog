...
# crontab定时任务
使用方法：
先编辑：`crontab -e` 打开文件 → 按格式写入任务 → 保存退出自动生效

crontab 时间格式为 `分 时 日 月 周`，每个字段取值：

| 字段 | 范围 | 特殊符号 |
|------|------|----------|
| 分   | 0-59 | `*` 任意, `*/N` 每N, `,` 多个, `-` 范围 |
| 时   | 0-23 | 同上 |
| 日   | 1-31 | 同上 |
| 月   | 1-12 | 同上 |
| 周   | 0-7 (0和7都表示周日) | 同上 |
...
# 示例:
*/5 * * * * /script.sh        # 每5分钟执行
0 9 * * * /script.sh           # 每天9:00执行
0 0 * * 1 /script.sh           # 每周一0:00执行
30 18 1 * * /script.sh         # 每月1号18:30执行
0 8,20 * * * /script.sh        # 每天8:00和20:00执行
0 2 * * 1-5 /script.sh         # 周一至周五2:00执行
@reboot /script.sh             # 开机启动
@daily /script.sh              # 每天执行（同 0 0 * * *）
```
